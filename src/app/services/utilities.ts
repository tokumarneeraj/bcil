...
export class Utilities {
  ...
  public static async b64toBlob(b64Data: string, contentType: string, sliceSize = 512): Promise<Blob> {
  ...
  public static dateFormateYYYYmmdd(param: string): string {
    const at = new Date(param); // .split('-');
    const mt = at.getMonth() < 9 ? '0' + (at.getMonth() + 1) : at.getMonth();
    const dt = at.getDate() < 10 ? '0' + at.getDate() : at.getDate();
    const newAt = at.getFullYear() + '-' + mt + '-' + dt;
    return newAt;

  }
  public static dateFormatemmddYYYY(param: string): string {
    const at = new Date(param); // .split('-');
    const mt = at.getMonth() < 9 ? '0' + (at.getMonth() + 1) : at.getMonth();
    const dt = at.getDate() < 10 ? '0' + at.getDate() : at.getDate();
    const newAt = mt + '-' + dt + '-' + at.getFullYear();
    return newAt;
  }
  public static dateFormateddmmyyyy(param: string): string {
    const at = new Date(param); // .split('-');
    const mt = at.getMonth() < 9 ? '0' + (at.getMonth() + 1) : at.getMonth();
    const dt = at.getDate() < 10 ? '0' + at.getDate() : at.getDate();
    const newAt = dt + '-' + mt + '-' + at.getFullYear();
    return newAt;
  }
  public static getDropDownText(id: any, object: any): any {
