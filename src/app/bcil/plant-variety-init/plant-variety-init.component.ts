...
@Component({
  selector: 'app-plant-variety-init',
  templateUrl: './plant-variety-init.component.html',
  styleUrls: ['./plant-variety-init.component.scss']
})
export class PlantVarietyInitComponent implements OnInit {

  showpage:boolean;
  plant_varietydata:any;
  type:string;
  UserId: string;
  userRoles: string[];
  isAdmin: boolean;
  ...
  constructor(private route: ActivatedRoute,private alertService: AlertService,private accountService: AccountService,private Bdoservice:Bdoservice,private router:Router
    ...
    viewadditionalfile(data:any){
      this.AdditionFile.showviewmodel(data,true,"plant_variety");
    }
    ...
    onmodalclick(e: string,value:any, data: any) {
      this.activity.showviewmodel('plant_variety',value,data);
    ...
    this.isSuperAdmin=this.userRoles.includes('Super Admin');
      ...
      this.viewtab=this.commondata.getotherpermissiondata('view').map((item)=>(item.split('-')[1]));
          ...
          this.Bdoservice.getdatapermission().subscribe(data=>{
            console.log(data);
            this.plant_varietydata=data?.plant_variety?.filter(x=>this.viewtab.find(y=>y==x.value));
        ...
        this.route.queryParams.subscribe((params) => {
          ...
          this.array=this.plant_varietydata?.find(x => x.type == params.type);
      ...
      this.Bdoservice.GetPlantvarietyModel().subscribe(data => {
        if(this.isSuperAdmin){
          this.plant_varietyModel = data.filter(x => x.app_Status == this.array?.value);
        }
    
  
      else {
         this.plant_varietyModel= data.filter(x=>x.app_Status== this.array?.value && this.activeusermou?.some(t=>t.appref==x.refid));
         }
